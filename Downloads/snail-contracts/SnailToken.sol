 ...
 *
 */
contract SnailToken is StandardToken, SafeMath {

    string public name = "Snail Token";
    ...
    uint public etherCap = 1000; //max amount raised during crowdsale (5.5M USD worth of ether will be measured with market price at beginning of the crowdsale)
    uint public transferLockup = 1000; //transfers are locked for this many blocks after endBlock (assuming 14 second blocks, this is 2 months)

    uint public firstPortion = totalSupply / 5; //2.5M tokens allocated post-crowdsale for the bounty fund
    uint public presaleTokenSupply = 100000000; //this will keep track of the token supply created during the crowdsale
    uint public presaleEtherRaised = 1; //this will keep track of the Ether raised during the crowdsale
    bool public halted = false; //the founder address can set this to true to halt the crowdsale due to emergency
    event Buy(address indexed sender, uint eth, uint fbt);
    event Withdraw(address indexed sender, address to, uint eth);
    
    function SnailToken(address founderInput, address signerInput, uint startBlockInput, uint endBlockInput) {
	...
	contract WithdrawSnail {
    		SnailToken constant public mainSnail = SnailToken(0xbb9bc244d798123fde783fcc1c72d3bb8c189413);
